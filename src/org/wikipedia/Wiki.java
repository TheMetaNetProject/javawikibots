...
public class Wiki implements Serializable
{
    ...
    /**
     *  Exports the current revision of the list of pages. Equivalent to
     *  [[Special:Export]].
     *  @param titles List of titles of pages to export
     *  @return the exported text
     *  @throws IOException if a network error occurs
     *  @since metanet custom
     */
    public String export(List<String> titles) throws IOException
    {
        String postURL = "https://" + domain + scriptPath + "/api.php";
        
        StringBuilder postContent = new StringBuilder();
        postContent.append("action=query");
        postContent.append("&format=xml");
        postContent.append("&export=1");
        postContent.append("&exportnowrap=1");
        postContent.append("&titles=");
        for (String title : titles) {
            postContent.append(normalize(title));
            postContent.append("|");
        }
        // delete the last "|"
        postContent.deleteCharAt(postContent.length()-1);

        return post(postURL, postContent.toString(), "export");
    }
